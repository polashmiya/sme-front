...
const stockInOut = months.map((m) => ({
  name: m,
  in: Math.floor(Math.random() * 400) + 600,
  out: Math.floor(Math.random() * 350) + 500,
}));
...
const topItemsByValue = Array.from({ length: 10 }).map((_, i) => ({
  name: `Item ${i + 1}`,
...
const movementTrend = months.map((m) => ({
  name: m,
  in: Math.floor(Math.random() * 500) + 500,
  out: Math.floor(Math.random() * 450) + 400,
  transfer: Math.floor(Math.random() * 200) + 100,
}));
...
export default function InventoryDashboardPage() {
  ...
  return (
    <>
      <HeaderWithOutCard
        title={t("inventory.dash.title", {
          defaultValue: "Inventory Dashboard",
        })}
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <ListCard
            title={t("inventory.dash.lists.lowStockAlerts")}
            items={[
              {
                id: "LS1",
                label: "Item A - 10 pcs left",
                status: "Critical",
                to: "/configuration/item-profile",
              },
              {
                id: "LS2",
                label: "Item F - 5 pcs left",
                status: "Critical",
                to: "/configuration/item-profile",
              },
            ]}
          />
          <ListCard
            title={t("inventory.dash.lists.overstockAlerts")}
            items={[
              {
                id: "OS1",
                label: "Item B - 5,000 pcs (limit 3,000)",
                status: "Alert",
                to: "/configuration/item-profile",
              },
              {
                id: "OS2",
                label: "Item G - 4,200 pcs (limit 3,000)",
                status: "Warning",
                to: "/configuration/item-profile",
              },
            ]}
          />
          <ListCard
            title={t("inventory.dash.lists.nearExpiryItems")}
            items={[
              {
                id: "NE1",
                label: "Item C - Expiry in 15 days",
                status: "Expiring",
                to: "/configuration/item-profile",
              },
              {
                id: "NE2",
                label: "Item H - Expiry in 22 days",
                status: "Expiring",
                to: "/configuration/item-profile",
              },
            ]}
          />
          <ListCard
            title={t("inventory.dash.lists.recentAdjustments")}
            items={[
              {
                id: "AD1",
                label: "Item D - Damaged - ৳ 3,000",
                status: "Adjusted",
                to: "/inventory/adjustment",
              },
              {
                id: "AD2",
                label: "Item I - Count Correction - ৳ 1,500",
                status: "Adjusted",
                to: "/inventory/adjustment",
              },
            ]}
          />
          <ListCard
            title={t("inventory.dash.lists.recentTransfers")}
            items={[
              {
                id: "TR1",
                label: "WH1 → WH2 - 200 pcs",
                status: "Transferred",
                to: "/inventory/transfer",
              },
              {
                id: "TR2",
                label: "WH2 → WH3 - 150 pcs",
                status: "Transferred",
                to: "/inventory/transfer",
              },
            ]}
          />
          <ListCard
            title={t("inventory.dash.lists.slowMovingItems")}
            items={[
              {
                id: "SM1",
                label: "Item E - No movement in 30 days",
                status: "Slow",
                to: "/configuration/item-profile",
              },
              {
                id: "SM2",
                label: "Item J - No movement in 25 days",
                status: "Slow",
                to: "/configuration/item-profile",
              },
            ]}
  ...
  return (
    ...
    >
      <h2 className="text-xs font-semibold mb-3 flex items-center gap-2">
        <Activity size={14} className="text-gray-400" /> {title}
      </h2>
