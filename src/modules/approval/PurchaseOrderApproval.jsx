...
// Generate 160+ dummy purchase order approvals
...
const rows = Array.from({ length: 160 }).map((_,i)=> ({
  ...
  receDate: new Date(2025, 1, (i%28)+1),
  ...
  return (
    <motion.div className="flex flex-col gap-4" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <motion.div className="mb-2 flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
        ...
        <span className="text-lg font-bold text-gray-800">Purchase Order Approval</span>
      </motion.div>
            ...
            {pageRows.map((r,idx) => (
              ...
              >
                <>
                  ...
                  <td>{r.poDate.toISOString().split('T')[0]}</td>
                  <td>{r.receDate.toISOString().split('T')[0]}</td>
                  <td>{r.poNo}</td>
                  <td>{r.supplier}</td>
                  <td>{r.createdBy}</td>
