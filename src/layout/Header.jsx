...
export default function Header() {
  ...
  return (
    <header className="header-area bg-white border-b border-gray-200 px-4 flex items-center justify-between sticky top-0 z-20 w-full" style={{ minWidth: 0 }}>
      ...
      <div className="flex items-center gap-2">
        {isMobile ? (
          <>
            <button className="flex items-center justify-center w-9 h-9 rounded-full bg-blue-100" onClick={() => setShowUser(v => !v)}>
            ...
            {showUser && (
              <div className="absolute right-4 top-14 bg-white rounded-xl shadow-lg border border-gray-200 p-4 z-50 min-w-[180px] flex flex-col items-center">
                ...
                <button className="btn-primary w-full mt-2" onClick={() => dispatch(signOut())}><LogOut size={16} /> {t('auth.signOut')}</button>
              </div>
            )}
          </>
        ) : (
