...
import { ShoppingCart, Package, Wallet, CreditCard, BarChart3, TrendingUp, AlertTriangle, FileText } from 'lucide-react'
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, PieChart, Pie, Cell } from 'recharts'

const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
const salesPurchaseTrend = months.map(m => ({ name: m, sales: Math.floor(Math.random()*9000)+1000, purchase: Math.floor(Math.random()*7000)+800 }))
const topItems = Array.from({ length: 5 }).map((_,i)=>({ name: `Item ${i+1}`, value: Math.floor(Math.random()*5000)+500 }))
const topCustomers = Array.from({ length: 5 }).map((_,i)=>({ name: `Customer ${i+1}`, value: Math.floor(Math.random()*4000)+400 }))
const topSuppliers = Array.from({ length: 5 }).map((_,i)=>({ name: `Supplier ${i+1}`, value: Math.floor(Math.random()*3500)+350 }))
...
const cashFlowTrend = months.map(m => ({ name: m, inflow: Math.floor(Math.random()*10000)+5000, outflow: Math.floor(Math.random()*8000)+3000 }))
  ...
  {kpiData().map((k,i)=>(
          ...
          >
            <div className="flex items-center gap-2 text-xs font-medium text-gray-600">
              <k.icon size={16} className="text-blue-600" /> {t(`dashboard.kpis.${k.key}`)}
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <ListCard title={t('dashboard.lists.pendingApprovals')} icon={<AlertTriangle size={16} className="text-yellow-600" />} items={[
          { id:'PO-1001', label:'PO-1001 - Supplier X - ৳ 9,500', to:'/approval' },
          { id:'SO-1023', label:'SO-1023 - Customer A - ৳ 12,000', to:'/approval' },
          { id:'PO-1002', label:'PO-1002 - Supplier Y - ৳ 7,200', to:'/approval' },
        ]} />
        <ListCard title={t('dashboard.lists.recentInvoices')} icon={<FileText size={16} className="text-blue-600" />} items={[
          { id:'INV-3001', label:'INV-3001 - Customer B - ৳ 15,400', to:'/sales/order' },
          { id:'INV-3002', label:'INV-3002 - Customer C - ৳ 11,200', to:'/sales/order' },
          { id:'INV-3003', label:'INV-3003 - Customer D - ৳ 9,800', to:'/sales/order' },
        ]} />
        <ListCard title={t('dashboard.lists.lowStock')} icon={<Package size={16} className="text-red-600" />} items={[
          { id:'ITM-1', label:'Item A (10 pcs left)', to:'/inventory/adjustment' },
          { id:'ITM-2', label:'Item B (5 pcs left)', to:'/inventory/adjustment' },
          { id:'ITM-3', label:'Item C (2 pcs left)', to:'/inventory/adjustment' },
        ]} />
      </div>
    </div>
  )
...
function ListCard({ title, icon, items }) {
  return (
    <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} className="card flex flex-col">
      <h2 className="text-xs font-semibold mb-3 flex items-center gap-2">{icon} {title}</h2>
      <ul className="space-y-2 text-sm">
        {items.map(it => (
          <li key={it.id}>
            <a href={it.to} className="block px-3 py-2 rounded-md border border-gray-200 hover:bg-gray-50 transition-colors">
              {it.label}
            </a>
          </li>
        ))}
