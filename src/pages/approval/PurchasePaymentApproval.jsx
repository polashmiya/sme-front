...
const rows = Array.from({ length: 160 }).map((_,i)=> ({
  id: i+1,
  paymentNo: 'PP-'+String(2000+i),
  paymentDate: new Date(2025, 0, (i%28)+1),
  supplier: SUPPLIERS[i % SUPPLIERS.length],
  createdBy: users[i % users.length],
  amount: Math.floor(Math.random()*30000)+3000,
}));

export default function PurchasePaymentApproval() {
  ...
  <motion.div className="mb-2 flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
        ...
        <span className="text-lg font-bold text-gray-800">Purchase Payment Approval</span>
  </motion.div>
          ...
          <tbody className="text-sm">
            {pageRows.map((r,idx) => (
              ...
              >
                ...
                <td>{r.paymentDate.toISOString().split('T')[0]}</td>
                <td>{r.paymentNo}</td>
                <td>{r.supplier}</td>
                <td>{r.createdBy}</td>
