...
const EXPENSES = ['Travel','Office Supplies','Utilities','Marketing'];
const users = ['User 1', 'User 2', 'User 3', 'User 4', 'User 5'];
const rows = Array.from({ length: 160 }).map((_,i)=> ({
  id: i+1,
  expenseNo: 'EXP-'+String(8000+i),
  expenseDate: new Date(2025, 0, (i%28)+1),
  expenseType: EXPENSES[i % EXPENSES.length],
  createdBy: users[i % users.length],
  amount: Math.floor(Math.random()*12000)+800,
}));

export default function ExpenseAdvanceApproval() {
  ...
  return (
    <motion.div className="flex flex-col gap-4" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <motion.div className="mb-2 flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
        ...
        <span className="text-lg font-bold text-gray-800">Expense/Advance Approval</span>
      </motion.div>
            ...
            {pageRows.map((r,idx) => (
              ...
              >
                <>
                  ...
                  <td>{r.expenseDate.toISOString().split('T')[0]}</td>
                  <td>{r.expenseNo}</td>
                  <td>{r.expenseType}</td>
                  <td>{r.createdBy}</td>
