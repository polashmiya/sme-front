...
const ACCOUNTS = ['Cash','Bank','Expense','Revenue'];
const users = ['User 1', 'User 2', 'User 3', 'User 4', 'User 5'];
const rows = Array.from({ length: 160 }).map((_,i)=> ({
  id: i+1,
  journalNo: 'JNL-'+String(7000+i),
  journalDate: new Date(2025, 0, (i%28)+1),
  account: ACCOUNTS[i % ACCOUNTS.length],
  createdBy: users[i % users.length],
  amount: Math.floor(Math.random()*15000)+1000,
}));

export default function AccountJournalApproval() {
  ...
  return (
    <motion.div className="flex flex-col gap-4" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <motion.div className="mb-2 flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
        ...
        <span className="text-lg font-bold text-gray-800">Account Journal Approval</span>
      </motion.div>
            ...
            {pageRows.map((r,idx) => (
              ...
              >
                <>
                  ...
                  <td>{r.journalDate.toISOString().split('T')[0]}</td>
                  <td>{r.journalNo}</td>
                  <td>{r.account}</td>
                  <td>{r.createdBy}</td>
